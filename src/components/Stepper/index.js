...
export function HorizontalNonLinearStepper(props) {
  ...
  const [subDetail, setSubDetails] = React.useState({
    quantity: "",
    startDate: "",
    endDate: "",
    noOfDays: "",
  ...
  const fetchData = async () => {
    ...
    const pSubArr = productsArray.filter((item) => {
      if (item.subscribable) return item;
    });
          ...
          <React.Fragment>
            ...
            <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
              <Button
                ...
                onClick={() => {
                  window.scrollTo({ left: 10, top: 500, behavior: "smooth" });
                  handleBack();
                }}
                sx={{ mr: 1 }}
              >
...
export default withStyles(styles)(
  connect(null, { addSubToCart, addToWishList })(HorizontalNonLinearStepper)
);
